    ...
    let closure = Closure::wrap(Box::new(move |time: f64| {
...

        {
            ...
            if let Err(err) = state.frame(dt) {
                ...
                state.hud.set_inner_text(&format!("WebGPU error\n{err:#}"));
...
fn canvas_size(canvas: &HtmlCanvasElement) -> (u32, u32) {
    ...
    let physical_width = (logical_width * dpr).round().max(1.0);
    let physical_height = (logical_height * dpr).round().max(1.0);
...
impl AppState {
    fn frame(&mut self, dt: f32) -> Result<()> {
        ...
        self.timer
            .update_hud(&self.hud, self.game.score, self.game.is_dead);

        self.resize_if_needed()?;

        let instances = self
            .game
            .instance_data(self.config.width, self.config.height);
    ...
    fn resize_if_needed(&mut self) -> Result<()> {
        ...
        if width > 0 && height > 0 && (width != self.config.width || height != self.config.height) {
            ...
            set_canvas_size(&self.canvas, width, height);
            self.surface.configure(&self.device, &self.config);
        }
        Ok(())
    }
...
impl FrameTimer {
    ...
    fn update_hud(&self, hud: &HtmlDivElement, score: i32, dead: bool) {
        ...
        hud.set_inner_text(&format!(
            "FPS: {:>5.1}\nScore: {}{}",
            self.fps, score, status
        ));
    }
}
